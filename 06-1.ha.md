...
listen kube-master
    ...
    server 192.168.1.111 192.168.1.111:6443 check inter 2000 fall 2 rise 2 weight 1
    server 192.168.1.112 192.168.1.112:6443 check inter 2000 fall 2 rise 2 weight 1
EOF
```
...
+ backup：192.168.1.111、192.168.1.112
...
``` bash
scp keepalived-master.conf root@192.168.1.110:/etc/keepalived/keepalived.conf
```

下发 backup 配置文件：

``` bash
scp keepalived-backup.conf root@192.168.1.111:/etc/keepalived/keepalived.conf
scp keepalived-backup.conf root@192.168.1.112:/etc/keepalived/keepalived.conf
```
